    ...
    <form action="" id="service-form">
        ...
        <div class="form-group">
            ...
            <input type="text" name="name" id="name" class="form-control form-control-border"
                placeholder="Enter Service" value="<?php echo isset($name) ? $name : '' ?>" required>
        </div>
        <div class="form-group">
            <label for="category_ids" class="control-label">For Category <small><em></em></small></label>
            <select name="category_ids[]" id="category_ids" class="form-control form-control-border select2" multiple>
                ...
                while ($row = $categories->fetch_assoc()):
                    ?>
                    <option value="<?= $row['id'] ?>" <?= isset($category_ids) && in_array($row['id'], explode(',', $category_ids)) ? "selected" : "" ?>     <?= $row['delete_flag'] == 1 ? "disabled" : "" ?>><?=
        ...
        <div class="form-group">
            ...
            <textarea rows="3" name="description" id="description"
                class="form-control form-control-sm rounded-0 summernote"
                data-placeholder="Write the service description here."
                required><?php echo isset($description) ? $description : '' ?></textarea>
        </div>
        <div class="form-group">
            <label for="fee" class="control-label">Fee</label>
            <input type="number" step="any" name="fee" id="fee" class="form-control form-control-border text-right"
                placeholder="Enter Fee" value="<?php echo isset($fee) ? $fee : 0 ?>" required>
        </div>
    </form>
</div>
<script>
