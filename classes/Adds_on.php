...
if(isset($_GET['fetchdata'])){
    ...
    foreach($data as $key => $row){
        ...
        <td>
      <?php
         $occ =  $conn->query("SELECT * FROM `appointment_list` where sched_id = '{$row['id']}' ")->num_rows;
        echo $occ;
      ?>
        ...
        <?php 
            if($occ >=1 ){
                echo '<span class="badge bg-success">Used</span>';
            }else {
                echo '<span class="badge bg-danger">Unused</span>';
            }
        
        ?>    
        </td>
        <td><button class="btn btn-light text-danger btn-sm btndelete"
        <?php 
            if($occ >=1 ){
              echo "disabled";
            }else {
...
function fetchdata(date){
  ...
  url: '../classes/Adds_on.php',
  method: 'GET',
  data: {update : 1, id:id,entity:entity,value:val },
  success: function(response) {
...
if(isset($_GET['update'])){
    ...
    if($entity == 'timestart'){
      $conn->query("UPDATE `scheduler` SET `timeend`= null WHERE id = '{$id}' ");
        $validate = $conn->query("SELECT * FROM `scheduler` WHERE  schedule = '{$datenow}' and '{$value}' BETWEEN timestart and timeend ");
